...
public class ChaseMode extends GhostState {
    ...
    public void computerNextDir() {
        int[] position = ghosts.getChaseTargetPosition(); // Target Pacman's position
        ...
        if (!ghosts.onTheGrid())
            return;
        if (!ghosts.onGameplayWindow())
            return;

        double minDist = Double.MAX_VALUE;

        if (ghosts.getXSpeed() <= 0 && !CheckCollision.checkWallCollision(ghosts, -ghosts.getSpeed(), 0)) {
            double distance = Utils.getDistance(ghosts.getXPos() - ghosts.getSpeed(), ghosts.getYPos(), position[0],
                    position[1]);
            if (distance < minDist) {
        ...
        if (ghosts.getYSpeed() <= 0 && !CheckCollision.checkWallCollision(ghosts, 0, -ghosts.getSpeed())) {
            double distance = Utils.getDistance(ghosts.getXPos(), ghosts.getYPos() - ghosts.getSpeed(), position[0],
                    position[1]);
            if (distance < minDist) {
                newXSpeed = 0;
